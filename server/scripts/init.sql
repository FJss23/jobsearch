...
CREATE TABLE IF NOT EXISTS jobsearch.appuser(
    ...
    locked varchar(50) NOT NULL CHECK (locked <> ''),
    enabled varchar(50) NOT NULL CHECK (enabled <> ''),
    logged_at timestamp,
    company_id integer,
...
CREATE TABLE IF NOT EXISTS jobsearch.registration_token(
    registration_token_id integer GENERATED BY DEFAULT AS IDENTITY,
    token varchar(100),
    ...
    FOREIGN KEY (appuser_id) REFERENCES jobsearch.appuser (appuser_id)
);
